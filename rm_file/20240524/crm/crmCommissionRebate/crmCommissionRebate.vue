<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="返利详情" prop="rebateDetails" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmCommissionRebateFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="返利详情">
                        {{ formData.rebateDetails }}
                </el-descriptions-item>
        </el-descriptions>
    </el-drawer>
  </div>
...
import {
  ...
  deleteCrmCommissionRebate,
  deleteCrmCommissionRebateByIds,
  updateCrmCommissionRebate,
  findCrmCommissionRebate,
  getCrmCommissionRebateList
...
const getTableData = async() => {
  const table = await getCrmCommissionRebateList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmCommissionRebateFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmCommissionRebateByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmCommissionRebateFunc = async(row) => {
    const res = await findCrmCommissionRebate({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCrmCommissionRebateFunc = async (row) => {
    const res = await deleteCrmCommissionRebate({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
