<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="合同ID" prop="contractId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmPurchaseOrderFunc(scope.row)">变更</el-button>
       ...
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="合同ID:"  prop="contractId" >
              <el-input v-model.number="formData.contractId" :clearable="true" placeholder="请输入合同ID" />
            </el-form-item>
...
<script setup>
import {
  createCrmPurchaseOrder,
  deleteCrmPurchaseOrder,
  deleteCrmPurchaseOrderByIds,
  updateCrmPurchaseOrder,
  findCrmPurchaseOrder,
  getCrmPurchaseOrderList
} from '@/api/crm/crmPurchaseOrder'
...
const getTableData = async() => {
  const table = await getCrmPurchaseOrderList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmPurchaseOrderFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmPurchaseOrderByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmPurchaseOrderFunc = async(row) => {
    const res = await findCrmPurchaseOrder({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmPurchaseOrder
...
// 删除行
const deleteCrmPurchaseOrderFunc = async (row) => {
    const res = await deleteCrmPurchaseOrder({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmPurchaseOrder(formData.value)
                  break
                case 'update':
                  res = await updateCrmPurchaseOrder(formData.value)
                  break
                default:
                  res = await createCrmPurchaseOrder(formData.value)
                  break
