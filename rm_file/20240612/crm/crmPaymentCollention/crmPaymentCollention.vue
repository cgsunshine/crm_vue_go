  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmPaymentCollentionFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteCrmPaymentCollention,
  deleteCrmPaymentCollentionByIds,
  updateCrmPaymentCollention,
  findCrmPaymentCollention,
  getCrmPaymentCollentionList
  ...
  createdAt: [
    { validator: (rule, value, callback) => {
      if (searchInfo.value.startCreatedAt && !searchInfo.value.endCreatedAt) {
        ...
        auditingTime : [{ validator: (rule, value, callback) => {
        if (searchInfo.value.startAuditingTime && !searchInfo.value.endAuditingTime) {
          callback(new Error('请填写结束日期'))
        } else if (!searchInfo.value.startAuditingTime && searchInfo.value.endAuditingTime) {
          callback(new Error('请填写开始日期'))
        } else if (searchInfo.value.startAuditingTime && searchInfo.value.endAuditingTime && (searchInfo.value.startAuditingTime.getTime() === searchInfo.value.endAuditingTime.getTime() || searchInfo.value.startAuditingTime.getTime() > searchInfo.value.endAuditingTime.getTime())) {
          callback(new Error('开始日期应当早于结束日期'))
        } else {
          callback()
        }
...
const getTableData = async() => {
  const table = await getCrmPaymentCollentionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmPaymentCollentionFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmPaymentCollentionByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmPaymentCollentionFunc = async(row) => {
    const res = await findCrmPaymentCollention({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCrmPaymentCollentionFunc = async (row) => {
    const res = await deleteCrmPaymentCollention({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
