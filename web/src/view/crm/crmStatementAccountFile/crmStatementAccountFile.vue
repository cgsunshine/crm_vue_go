  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmStatementAccountFileFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createCrmStatementAccountFile,
  deleteCrmStatementAccountFile,
  deleteCrmStatementAccountFileByIds,
  updateCrmStatementAccountFile,
  findCrmStatementAccountFile,
  getCrmStatementAccountFileList
} from '@/api/crm/crmStatementAccountFile'
...
const getTableData = async() => {
  const table = await getCrmStatementAccountFileList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmStatementAccountFileFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmStatementAccountFileByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmStatementAccountFileFunc = async(row) => {
    const res = await findCrmStatementAccountFile({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmStatementAccountFile
...
// 删除行
const deleteCrmStatementAccountFileFunc = async (row) => {
    const res = await deleteCrmStatementAccountFile({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmStatementAccountFile(formData.value)
                  break
                case 'update':
                  res = await updateCrmStatementAccountFile(formData.value)
                  break
                default:
                  res = await createCrmStatementAccountFile(formData.value)
                  break
