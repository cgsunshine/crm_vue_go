<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="订购单ID" prop="purchaseOrderId" width="120" />
        <el-table-column align="left" label="金额" prop="amount" width="120" />
        <el-table-column align="left" label="账单文件" prop="billFile" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmStatementAccountFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="订购单ID:"  prop="purchaseOrderId" >
              <el-input v-model.number="formData.purchaseOrderId" :clearable="true" placeholder="请输入订购单ID" />
            </el-form-item>
            <el-form-item label="金额:"  prop="amount" >
              <el-input-number v-model="formData.amount"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
            <el-form-item label="账单文件:"  prop="billFile" >
              <el-input v-model="formData.billFile" :clearable="true"  placeholder="请输入账单文件" />
            </el-form-item>
            <el-form-item label="创建时间:"  prop="creationTime" >
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                <el-descriptions-item label="订购单ID">
                        {{ formData.purchaseOrderId }}
                </el-descriptions-item>
                <el-descriptions-item label="金额">
                        {{ formData.amount }}
                </el-descriptions-item>
                <el-descriptions-item label="账单文件">
                        {{ formData.billFile }}
                </el-descriptions-item>
                <el-descriptions-item label="创建时间">
                      {{ formatDate(formData.creationTime) }}
                </el-descriptions-item>
                <el-descriptions-item label="应付时间">
...
<script setup>
import {
  createCrmStatementAccount,
  deleteCrmStatementAccount,
  deleteCrmStatementAccountByIds,
  updateCrmStatementAccount,
  findCrmStatementAccount,
  getCrmStatementAccountList
} from '@/api/crm/crmStatementAccount'
...
const getTableData = async() => {
  const table = await getCrmStatementAccountList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmStatementAccountFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmStatementAccountByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmStatementAccountFunc = async(row) => {
    const res = await findCrmStatementAccount({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmStatementAccount
...
// 删除行
const deleteCrmStatementAccountFunc = async (row) => {
    const res = await deleteCrmStatementAccount({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmStatementAccount(formData.value)
                  break
                case 'update':
                  res = await updateCrmStatementAccount(formData.value)
                  break
                default:
                  res = await createCrmStatementAccount(formData.value)
                  break
