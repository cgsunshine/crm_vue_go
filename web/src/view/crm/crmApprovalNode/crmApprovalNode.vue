  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmApprovalNodeFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteCrmApprovalNode,
  deleteCrmApprovalNodeByIds,
  updateCrmApprovalNode,
  findCrmApprovalNode,
  getCrmApprovalNodeList
...
const getTableData = async() => {
  const table = await getCrmApprovalNodeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmApprovalNodeByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmApprovalNodeFunc = async(row) => {
    const res = await findCrmApprovalNode({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCrmApprovalNodeFunc = async (row) => {
    const res = await deleteCrmApprovalNode({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
