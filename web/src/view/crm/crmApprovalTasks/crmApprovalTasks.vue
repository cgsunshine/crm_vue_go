  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmApprovalTasksFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createCrmApprovalTasks,
  deleteCrmApprovalTasks,
  deleteCrmApprovalTasksByIds,
  updateCrmApprovalTasks,
  findCrmApprovalTasks,
  getCrmApprovalTasksList
} from '@/api/crm/crmApprovalTasks'
...
const getTableData = async() => {
  const table = await getCrmApprovalTasksList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmApprovalTasksFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmApprovalTasksByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmApprovalTasksFunc = async(row) => {
    const res = await findCrmApprovalTasks({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmApprovalTasks
...
// 删除行
const deleteCrmApprovalTasksFunc = async (row) => {
    const res = await deleteCrmApprovalTasks({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmApprovalTasks(formData.value)
                  break
                case 'update':
                  res = await updateCrmApprovalTasks(formData.value)
                  break
                default:
                  res = await createCrmApprovalTasks(formData.value)
                  break
