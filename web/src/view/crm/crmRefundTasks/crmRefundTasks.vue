  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="审批是否有效 1 有效 2 失效（多人审批中，有人拒绝）" prop="valid">
             ...
             <el-input v-model.number="searchInfo.valid" placeholder="搜索条件" />
        ...
        <el-form-item label="退款类型 1押金" prop="refundType">
             ...
             <el-input v-model.number="searchInfo.refundType" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="退款状态" prop="refundStatus" width="120">
            ...
            {{ filterDict(scope.row.refundStatus,refund_statusOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="补充意见" prop="comments" width="120" />
        ...
        <el-table-column align="left" label="退款类型 1押金" prop="refundType" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmRefundTasksFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="退款类型 1押金:"  prop="refundType" >
              <el-input v-model.number="formData.refundType" :clearable="true" placeholder="请输入退款类型 1押金" />
            </el-form-item>
          </el-form>
    </el-drawer>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="退款状态">
                        {{ filterDict(formData.refundStatus,refund_statusOptions) }}
                </el-descriptions-item>
                <el-descriptions-item label="补充意见">
                        {{ formData.comments }}
                </el-descriptions-item>
                ...
                <el-descriptions-item label="退款类型 1押金">
                        {{ formData.refundType }}
                </el-descriptions-item>
        </el-descriptions>
    </el-drawer>
...
import {
  ...
  deleteCrmRefundTasks,
  deleteCrmRefundTasksByIds,
  updateCrmRefundTasks,
  findCrmRefundTasks,
  getCrmRefundTasksList
...
const getTableData = async() => {
  const table = await getCrmRefundTasksList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmRefundTasksByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmRefundTasksFunc = async(row) => {
    const res = await findCrmRefundTasks({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCrmRefundTasksFunc = async (row) => {
    const res = await deleteCrmRefundTasks({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
