  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmContactApprovalTasksFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteCrmContactApprovalTasks,
  deleteCrmContactApprovalTasksByIds,
  updateCrmContactApprovalTasks,
  findCrmContactApprovalTasks,
  getCrmContactApprovalTasksList
...
const getTableData = async() => {
  const table = await getCrmContactApprovalTasksList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmContactApprovalTasksFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmContactApprovalTasksByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmContactApprovalTasksFunc = async(row) => {
    const res = await findCrmContactApprovalTasks({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCrmContactApprovalTasksFunc = async (row) => {
    const res = await deleteCrmContactApprovalTasks({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
