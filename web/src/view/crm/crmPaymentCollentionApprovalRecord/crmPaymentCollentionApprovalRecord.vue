  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmPaymentCollentionApprovalRecordFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteCrmPaymentCollentionApprovalRecord,
  deleteCrmPaymentCollentionApprovalRecordByIds,
  updateCrmPaymentCollentionApprovalRecord,
  findCrmPaymentCollentionApprovalRecord,
  getCrmPaymentCollentionApprovalRecordList
...
defineOptions({
    name: 'CrmPaymentCollentionApprovalRecord'
})
...
const getTableData = async() => {
  const table = await getCrmPaymentCollentionApprovalRecordList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmPaymentCollentionApprovalRecordFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmPaymentCollentionApprovalRecordByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmPaymentCollentionApprovalRecordFunc = async(row) => {
    const res = await findCrmPaymentCollentionApprovalRecord({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCrmPaymentCollentionApprovalRecordFunc = async (row) => {
    const res = await deleteCrmPaymentCollentionApprovalRecord({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
