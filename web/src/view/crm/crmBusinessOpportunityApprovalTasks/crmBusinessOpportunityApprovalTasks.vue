  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmBusinessOpportunityApprovalTasksFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createCrmBusinessOpportunityApprovalTasks,
  deleteCrmBusinessOpportunityApprovalTasks,
  deleteCrmBusinessOpportunityApprovalTasksByIds,
  updateCrmBusinessOpportunityApprovalTasks,
  findCrmBusinessOpportunityApprovalTasks,
  getCrmBusinessOpportunityApprovalTasksList
} from '@/api/crm/crmBusinessOpportunityApprovalTasks'
...
defineOptions({
    name: 'CrmBusinessOpportunityApprovalTasks'
})
...
const getTableData = async() => {
  const table = await getCrmBusinessOpportunityApprovalTasksList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteCrmBusinessOpportunityApprovalTasksFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmBusinessOpportunityApprovalTasksByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmBusinessOpportunityApprovalTasksFunc = async(row) => {
    const res = await findCrmBusinessOpportunityApprovalTasks({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmBusinessOpportunityApprovalTasks
...
// 删除行
const deleteCrmBusinessOpportunityApprovalTasksFunc = async (row) => {
    const res = await deleteCrmBusinessOpportunityApprovalTasks({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmBusinessOpportunityApprovalTasks(formData.value)
                  break
                case 'update':
                  res = await updateCrmBusinessOpportunityApprovalTasks(formData.value)
                  break
                default:
                  res = await createCrmBusinessOpportunityApprovalTasks(formData.value)
                  break
