<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="公司名称" prop="commpanyName" width="120" />
        ...
        <el-table-column align="left" label="电话" prop="telephone" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmSupplierFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="公司名称:"  prop="commpanyName" >
              <el-input v-model="formData.commpanyName" :clearable="true"  placeholder="请输入公司名称" />
            </el-form-item>
            <el-form-item label="联系人:"  prop="contactPerson" >
            ...
            <el-form-item label="电话:"  prop="telephone" >
              <el-input v-model="formData.telephone" :clearable="true"  placeholder="请输入电话" />
            </el-form-item>
            <el-form-item label="备注添加时间:"  prop="noteAddTime" >
...
import {
  createCrmSupplier,
  deleteCrmSupplier,
  deleteCrmSupplierByIds,
  updateCrmSupplier,
  findCrmSupplier,
  getCrmSupplierList
} from '@/api/crm/crmSupplier'
...
const getTableData = async() => {
  const table = await getCrmSupplierList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmSupplierByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmSupplierFunc = async(row) => {
    const res = await findCrmSupplier({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmSupplier
...
// 删除行
const deleteCrmSupplierFunc = async (row) => {
    const res = await deleteCrmSupplier({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmSupplier(formData.value)
                  break
                case 'update':
                  res = await updateCrmSupplier(formData.value)
                  break
                default:
                  res = await createCrmSupplier(formData.value)
                  break
