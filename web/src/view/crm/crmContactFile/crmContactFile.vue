  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCrmContactFileFunc(scope.row)">变更</el-button>
...
<script setup>
import {
  createCrmContactFile,
  deleteCrmContactFile,
  deleteCrmContactFileByIds,
  updateCrmContactFile,
  findCrmContactFile,
  getCrmContactFileList
} from '@/api/crm/crmContactFile'
...
const getTableData = async() => {
  const table = await getCrmContactFileList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCrmContactFileByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCrmContactFileFunc = async(row) => {
    const res = await findCrmContactFile({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recrmContactFile
...
// 删除行
const deleteCrmContactFileFunc = async (row) => {
    const res = await deleteCrmContactFile({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCrmContactFile(formData.value)
                  break
                case 'update':
                  res = await updateCrmContactFile(formData.value)
                  break
                default:
                  res = await createCrmContactFile(formData.value)
                  break
