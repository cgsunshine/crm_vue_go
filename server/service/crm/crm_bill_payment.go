...
// Author [piexlmax](https://github.com/piexlmax)
func (crmBillPaymentService *CrmBillPaymentService) GetCrmBillPayment(ID string) (crmBillPayment crm.CrmBillPayment, err error) {
	err = global.GVA_DB.Where("id = ?", ID).First(&crmBillPayment).Error
	return
}
