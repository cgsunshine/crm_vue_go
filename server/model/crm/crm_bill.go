...
type CrmBill struct {
	global.GVA_MODEL
	OrderId             *int       `json:"orderId" form:"orderId" gorm:"column:order_id;comment:订单ID;" binding:"required"`                    //订单ID
	...
	Currency            string     `json:"currency" form:"currency" gorm:"column:currency;comment:币种;size:11;" binding:"required"`            //币种
	UserId              *int       `json:"userId" form:"userId" gorm:"column:user_id;comment:用户id;"`                                          //管理ID 销售代表
	BillNumber          string     `json:"billNumber" form:"billNumber" gorm:"column:bill_number;comment:账单编号;size:191;" binding:"required"`  //账单编号
