...
type CrmPurchaseOrder struct {
 global.GVA_MODEL 
      ContractId  *int `json:"contractId" form:"contractId" gorm:"column:contract_id;comment:合同ID;size:10;"`  //合同ID 
      ProductId  *int `json:"productId" form:"productId" gorm:"column:product_id;comment:产品ID;size:10;"`  //产品ID 
      Quantity  *int `json:"quantity" form:"quantity" gorm:"column:quantity;comment:数量;size:10;"`  //数量 
      Amount  *float64 `json:"amount" form:"amount" gorm:"column:amount;comment:金额;size:10;"`  //金额 
      UserId  *int `json:"userId" form:"userId" gorm:"column:user_id;comment:负责人ID;size:10;"`  //负责人ID 
